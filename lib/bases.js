...
/**
 * Check if a base is premium (locked behind content locker)
 * First base always premium, then randomly scattered
 */
export function isPremiumBase(base) {
  ...
  // Numbers ending in 7 are premium (67, 57, 47, 37, 27, 17, 7)
  // This makes first TH18 base (67) premium
  // Plus some other scattered numbers for variety
  const lastDigit = baseNumber % 10;
  const secondLastDigit = Math.floor(baseNumber / 10) % 10;

  // ~15% premium: numbers ending in 7, or ending in 3 with even tens digit
  if (lastDigit === 7) return true;
  if (lastDigit === 3 && secondLastDigit % 2 === 0) return true;
  if (lastDigit === 1 && secondLastDigit === 4) return true; // 41

  return false;
}

/**
 * Get all premium bases
 */
export function getPremiumBases() {
  const allBases = getAllBases();
  return allBases.filter(isPremiumBase);
}

/**
 * Get premium base count
 */
export function getPremiumCount() {
  return getPremiumBases().length;
}
