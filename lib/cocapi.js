...
// Uses ClashKing proxy (no IP restrictions) or direct API for local dev

const USE_CLASHKING = process.env.NODE_ENV === 'production' || process.env.USE_CLASHKING === 'true';
const COC_API_BASE = USE_CLASHKING
  ? 'https://proxy.clashking.xyz/v1'
  : 'https://api.clashofclans.com/v1';

// Get API key (only needed for direct API access)
function getApiKey() {
  if (USE_CLASHKING) return null;
  const apiKey = process.env.COC_API_KEY;
  if (!apiKey) {
    throw new Error('COC_API_KEY not set. Get key from https://developer.clashofclans.com/');
  }
  return apiKey;
}
