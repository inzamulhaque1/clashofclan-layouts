...
// Use proxy if set, otherwise direct API (for local dev)
const COC_API_BASE = process.env.COC_API_PROXY || 'https://api.clashofclans.com/v1';
const USE_PROXY = !!process.env.COC_API_PROXY;

// Get API key from environment (not needed if using proxy)
function getApiKey() {
  if (USE_PROXY) return null; // Proxy handles auth
  const apiKey = process.env.COC_API_KEY;
  if (!apiKey) {
    throw new Error('COC_API_KEY environment variable is not set. Get your key from https://developer.clashofclans.com/');
  }
  return apiKey;
}
...
async function cocFetch(endpoint, options = {}) {
  ...
  const url = USE_PROXY ? `${COC_API_BASE}/v1${endpoint}` : `${COC_API_BASE}${endpoint}`;
